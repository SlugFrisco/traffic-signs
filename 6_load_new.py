...
# Take in output of conv 2, make the flat layer
layer_flat, num_features = flatten_layer(layer_conv2)

# Make fully connected layer, takes in output of flat layer, output is # of neurons
...
# Make fully connected layer 2, which takes in 128 inputs and outputs a vector of n_classes logits
...
print(softmax_img)
print("type: {}".format(type(softmax_img)))
print(np.argmax(softmax_img))
print(softmax_img[0][np.argmax(softmax_img)])
print(get_text(np.argmax(softmax_img)))
#values, indices = tf.nn.top_k(softmax_img, k=1, sorted=True, name=None)
#print(values)
#print(indices)
