...
learning_rate = 0.005
training_epochs = 1
iterations = 1000    # actual number of iterations run per epoch will be min(this number, # of batches per epoch)
train_batch_size = 50
test_batch_size = 50
display_step = 10


# Helper: Shuffle and batch function
def shuffle_and_batch(features, labels, batch_size):
    ...
    return feature_batches, label_batches, total_batch
...
# Make fully connected layer 2, which takes in 128 things and outputs a vector of logits
logits = new_fc_layer(input = dropout,
                         num_inputs = fc_size,
...
def print_test_accuracy(session):
    ...
    while i < num_test:
        ...
        feed_dict = {x: images,
                     y_true: labels,
                     keep_prob: 1} # remember to set test keep_prob to 1
...
with tf.Session() as sess:
    ...
    for i in range(0, training_epochs):
        # shuffle and generate new batches per epoch
        feature_batches, label_batches, total_batch = shuffle_and_batch(train['features'],
                                                                        one_hot_train_labels,
                                                                        train_batch_size)
        for j in range(0, min(iterations, total_batch)):
            # grab a batch
            ...
            train_feed_dict = {x: batch_x,
                               y_true: batch_y,
                               keep_prob: 0.5}
            one_feed_dict = {x: batch_x,
                             y_true: batch_y,
                             keep_prob: 1} # don't use dropout to backpropagate or calculate accuracy/cost
            # Run optimization op (backprop) and cost op (to get loss value)
            sess.run(optimizer, feed_dict=train_feed_dict)
            # Display logs per iteration step
            if (j + 1) % display_step == 0:
                c = sess.run(cost, feed_dict=one_feed_dict)
                acc = sess.run(accuracy, feed_dict=one_feed_dict)
                print("Epoch {} / Iteration {}:\tCost={:.5},\tTraining accuracy={:.1%}".format(i+1, j+1, c, acc))
