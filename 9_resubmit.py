...
image_list = ["school.jpg",
              ...
              "80.jpg",
              "40.jpg"]
labels_list = [28,  # "Children crossing"
               14,  # "Stop"
               14,  # "Stop"
               16,  # "Vechiles over 3.5 metric tons prohibited" (closest, visually)
               7,   # "Speed limit (100km/h)"
               7,   # "Speed limit (100km/h)"
               3,   # "Speed limit (60km/h)"
               5,   # "Speed limit (80km/h)"
               2    # "Speed limit (40km/h)" # granted 40 doesn't exist in the original set,
                    # so I'm just going to put in the number for 50km/h
               ]

correct_count = 0
top_five_count = 0
for i in range(0, len(image_list)):
    prediction = predict(sess, image_list[i], top_k=True)
    if prediction[0] == labels_list[i]:
        correct_count += 1
    if labels_list[i] in prediction:
        top_five_count += 1

print("Accuracy of predictions: {:.0f}%".format(float(correct_count) * 100 / (float(len(labels_list)))))
print("Accuracy of predictions in top 5: {:.0f}%".format(float(top_five_count) * 100 / (float(len(labels_list)))))
